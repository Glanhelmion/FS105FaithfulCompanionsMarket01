...
// Accessory routes
accessoryRouter
  .route("/") // Using .route() for the root path
  .get(getAccessories); // Define the route handler for GET request

accessoryRouter
  .route("/:id") // Using .route() for the path with ID parameter
  .get(getAccessoryById); // Define the route handler for GET request with ID

// Bird routes
birdRouter.route("/").get(getBirds);

birdRouter.route("/:id").get(getBirdById);
...
// Dog routes
dogRouter.route("/").get(getDogs);

dogRouter.route("/:id").get(getDogById);

// Fish routes
fishRouter.route("/").get(getFishes);

fishRouter.route("/:id").get(getFishById);

// Petfood routes
petfoodRouter.route("/").get(getPetfoods);

petfoodRouter.route("/:id").get(getPetfoodById);
